...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
        ...
        if (user.isPresent()) {
            ...
            if (serviceUser.checkPassword(loginRequest.getPassword(), user.get().getPassword())) {
                // Chọn vai trò đầu tiên hoặc mặc định là "USER"
                String role = user.get().getRoles().isEmpty() ? "USER" : user.get().getRoles().get(0);
                String token = jwtUtil.generateToken(user.get().getEmail(), role);
                System.out.println("🔑 Token đã tạo: " + token);
                return ResponseEntity.ok(new AuthResponse(token));
            } else {
                System.out.println("❌ Mật khẩu không khớp!");
