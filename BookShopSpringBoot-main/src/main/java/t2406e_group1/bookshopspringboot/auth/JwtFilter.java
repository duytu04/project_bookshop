...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // Bỏ qua yêu cầu OPTIONS
        if ("OPTIONS".equalsIgnoreCase(method)) {
            logger.debug("Bypassing JwtFilter for OPTIONS request: {}", requestURI);
            filterChain.doFilter(request, response);
            return;
        }
        ...
        if (requestURI.startsWith("/api/auth/login")
            || requestURI.startsWith("/api/discounts")
            || requestURI.startsWith("/api/supplier")
            || requestURI.startsWith("/api/import-products")
            || requestURI.startsWith("/api/review")
            || requestURI.startsWith("/api/product")
            || requestURI.startsWith("/api/orders")
            || ("POST".equalsIgnoreCase(method) && requestURI.equals("/api/user"))) {
            logger.debug("Bypassing JwtFilter for public endpoint: {}", requestURI);
            filterChain.doFilter(request, response);
            return;
        }
